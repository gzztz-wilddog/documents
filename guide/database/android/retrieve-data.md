...
更多更详细的用法说明参见 [API](/docs/guide/database/android/api.html#DataSnapshot-Methods) 文档。

#### child 事件
当某个节点的子节点发生改变时（如通过 `push()` 方法添加子节点，或通过 `updateChildren()` 更新子节点），就会触发 `child 事件`。
对于监听对数据库中某个特定节点的子节点所做的各种变动，结合使用上述每一种方法就显得非常有用。

`onChildAdded()` 事件常用来获取当前路径下的子节点列表。初始化时会触发一次并获取所有子节点，之后每当增加子节点时就会再次触发获取新增的子节点。
